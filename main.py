...
async def command_start(message: types.Message, state: FSMContext):
    car_flag = await db.check_car(message.from_user.id)
    if ("TANK300" in message.text or "TANK500" in message.text) and not car_flag:
        ...
        media = types.MediaGroup()
        media.attach_photo(types.InputMediaPhoto(open("pamyatka.jpg", 'rb'),
                                                 caption=f"Я уже начал подбирать собеседника, с которым вы вместе"
                                                         f" сможете попробовать авторские коктейли от TANK.\n\n"
                                                         f"Не отключайте уведомления и ознакомьтесь с"
                                                         f" инструкцией на картинке:"))
        await bot.send_media_group(message.from_user.id, media)
        ...
        if res_user != 0:
            ...
            if len(themes[0]) == 1:
                themes = [['Путешествия', 'Еда', 'Работа и карьера', 'Досуг в Москве', 'Спорт и увлечения']]
                await db.update_themes(message.from_user.id, themes[0])
            ...
            await bot.send_message(user_tg_id,
                                   ...
                                   f"Если возникнут трудности, ты всегда можешь обратиться к организатору "
                                   f"или на стойку ресепшн к хостес за помощью.")
            await bot.send_message(user_tg_id, msg, reply_markup=actions_keyboard)
            await db.append_to_matches(message.from_user.id, user_tg_id)
            await db.append_to_matches(user_tg_id, message.from_user.id)
            await db.update_match_flag(message.from_user.id, True)
            await db.update_match_flag(user_tg_id, True)
    ...
    if message.text == "Сменить тему для разговора":
        ...
        media.attach_photo(types.InputMediaPhoto(open("pamyatka.jpg", 'rb'),
                                                 caption=f"Я уже начал подбирать собеседника, с которым вы вместе"
                                                         ...
                                                         f" инструкцией на картинке:"))
        await bot.send_media_group(message.from_user.id, media)
        await db.update_match_flag(message.from_user.id, False)
